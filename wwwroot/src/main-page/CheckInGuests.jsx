    ...
    axios.get('/api/reservations')
      .then(response => {
        setReservations(response.data
          .filter(res => moment(res.checkinDate).format("YYYY-MM-DD") ===  moment(new Date()).format("YYYY-MM-DD")));
        setChecked(response.data.filter(res => res.reservationState === 1).map(res => res.id))
      });
  ...
  const handleToggle = (reservationId) => () => {
    const currentIndex = checked.indexOf(reservationId);
      ...
      if (currentIndex === -1) {
        ...
        updateReservation(reservationId, 1, 1);
      } else {
        newChecked.splice(currentIndex, 1);
        updateReservation(reservationId, 0, 0)
      }
